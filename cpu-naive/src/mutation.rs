...
pub trait Mutation {
    fn mutate(&mut self, solutions: &mut [Solution], data: &TimetableData);
}
...
impl BasicMutation {
    pub fn new(rng: Box<dyn Rng>, probability: f32) -> Self {
    ...
    fn mutate(&mut self, solutions: &mut [Solution], data: &TimetableData) {
        ...
        for sol in solutions.iter_mut() {
            for class_idx in 0..n_classes {
                if self.rng.random_range(0.0..1.0) <= self.probability {

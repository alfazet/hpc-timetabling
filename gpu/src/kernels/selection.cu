...
Selection::Selection(usize population_size, f32 frac, u32 tournament_size)
    : selected(std::ceil(population_size * frac)), frac(frac), tournament_size(tournament_size) {}

__global__ void k_tournament_select(u16 *selected, const u16 *order, usize n_selected, usize population_size,
                                    u32 tournament_size, u32 seed) {
    ...
    u16 winner_idx;
    if (lane < tournament_size) {
        curandState rng;
        curand_init(seed, warp_id * WARP_SIZE + lane, 0, &rng);
        winner_idx = curand(&rng) % population_size;
    } else {
        winner_idx = UINT16_MAX; // non-participating lanes "lose" automatically
    }
...
void Selection::select(const Population &population) {
    ...
    k_tournament_select<<<grid_dim, block_dim>>>(d_selected, d_order, n_selected, population.population_size,
                                                 this->tournament_size, seed);
